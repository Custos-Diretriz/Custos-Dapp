...
import type { Agreement, AgreementApi, AgreementOnchain, AgreementSlugParams } from "@/types/agreement";
const ReactQuill = dynamic(() => import("react-quill"), { ssr: false });
import "react-quill/dist/quill.snow.css";

const AgreementSlug = ({ params }: AgreementSlugParams) => {
  ...
  const [contentFormat, setContentFormat] = useState<string>("html");
  const { openNotification } = useNotification();
  const router = useRouter();
  const { address } = useWallet();
  const slug = params?.slug || [];
  const [key, value] = slug as [string, string];
  const { fetchData } = UseReadContractData();

  useEffect(() => {
    const fetchAgreementById = async (agreementId: string | number) => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/agreement/agreement/${agreementId}/`
        );
        if (response.ok) {
          const data: AgreementApi = await response.json();
          setAgreement(data as AgreementApi);
          initializeEditableFields(data as AgreementApi);
        } else {
    ...
    const fetchAgreementByAccessToken = async (token: string) => {
      try {
        const response = await fetch(
    ...
    const getOnchainAgreement = async (agreementId: string) => {
      try {
        const agreementDetails = await fetchData(
          "agreement",
          "get_agreement",
          [agreementId]
        );
        console.log("agreement details", agreementDetails);
        setAgreement(agreementDetails as AgreementApi);
        initializeEditableFields(agreementDetails as AgreementApi);
      } catch (error) {
        console.error("Error fetching onchain agreement:", error);
        openNotification("error", "Error fetching onchain agreement", `${error}`);
      } finally {
        setLoading(false);
      }
    };
    ...
    if (key === "access_token") {
      setAccessToken(value || params?.agreementAccessToken || null);
  ...
  }, [key, value, params?.agreementAccessToken, fetchData, openNotification]);
  ...
  const initializeEditableFields = (data: AgreementApi | AgreementOnchain) => {
    setEditableFields({
      content: (data as AgreementApi)?.content,
      email: (data as AgreementApi)?.email ?? "",
      first_party_country: (data as AgreementApi)?.first_party_country ?? "",
      first_party_id_type: (data as AgreementApi)?.first_party_id_type ?? "",
    });
  ...
  const handleSave = async () => {
    ...
    setLoading(true)
    openNotification("info", "Saving Agreement", "Please wait while we save your agreement...")
    try {
      ...
      if (response.ok) {
        const updatedAgreement: AgreementApi = await response.json();
        setLoading(false)
        openNotification("success", "Success", "Agreement updated successfully")
  ...
  const renderContent = (content: string = "") => {
    const sanitized = DOMPurify.sanitize(content, {
      ALLOWED_URI_REGEXP: /^(?:(?:https?|ipfs):|[^a-z]|[a-z+.-]+(?:[^a-z+.-]|$))/i,
      ...
      <div className="p-4 rounded-lg shadow-lg bg-gradient-to-r w-full">
          ...
          <div className="flex-col w-full items-end text-end gap-3 flex">
            ...
            <div className="w-full flex flex-col lg:flex-row gap-2 justify-end max-lg:items-start">
              ...
              <span className="bg-gradient-to-r from-[#19B1D2] to-[#0094FF] bg-clip-text text-left text-transparent">
                {key == "onchain"
                  ? (agreement as AgreementOnchain)?.created_at
                  : formatDate((agreement as AgreementApi)?.created_at || "")}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-col gap-2">
            ...
            {isEditing ? (
              ...
              <div className="py-4 rounded-lg font-normal text-sm">
                {renderContent((agreement as AgreementApi)?.content || (agreement as AgreementOnchain)?.content)}
              </div>
            )}
          </div>
          {key == "onchain" ? (
            ...
            <>
              <div className="flex flex-col gap-2">
                ...
                {isEditing ? (
                  ...
                  <span className="text-sm">{(agreement as AgreementApi).email || "N/A"}</span>
                )}
              </div>
            </>
          )}
          <div className="flex flex-col gap-2">
            ...
            <span className="text-sm">{(agreement as AgreementApi | AgreementOnchain)?.first_party_address}</span>
          </div>
          <div className="flex flex-col gap-2">
            <strong className="text-lg">First Party Valid ID:</strong>
            <Image
              src={(agreement as AgreementApi)?.first_party_valid_id || "/not-found-image.png"}
              alt="First Party ID"
              ...
              <div className="flex flex-col gap-2">
                ...
                {isEditing ? (
                  ...
                  <span className="text-sm">
                    {(agreement as AgreementApi)?.first_party_country || "N/A"}
                  </span>
                )}
              </div>
              <div className="flex flex-col gap-2">
                ...
                {isEditing ? (
                  ...
                  <span className="text-sm">
                    {(agreement as AgreementApi)?.first_party_id_type}
                  </span>
                )}
              </div>
              <div className="flex flex-col gap-2">
                ...
                <Image
                  src={(agreement as AgreementApi)?.first_party_signature || "/not-found-image.png"}
          ...
          {key == "onchain" ? (
            ...
            <>
              <div className="flex flex-col gap-2">
                ...
                <span className="text-sm">{(agreement as AgreementApi).second_party_address}</span>
              </div>
              <div className="flex flex-col gap-2">
                <strong className="text-lg">Second Party Valid ID:</strong>
                <span className="text-sm">
                  <Image
                    src={(agreement as AgreementApi)?.second_party_valid_id || "/not-found-image.png"}
                    alt="First Party Signature"
                    width={20}
                    height={20}
                    className="w-[16em] h-[10em] bg-white object-cover rounded-lg"
              ...
              <div className="flex flex-col gap-2">
                ...
                <span className="text-sm">
                  {(agreement as AgreementApi).second_party_country || "N/A"}
                </span>
              </div>
              <div className="flex flex-col gap-2">
                <strong className="text-lg">Second Party ID Type:</strong>
                <span className="text-sm">
                  {(agreement as AgreementApi).second_party_id_type || "N/A"}
                </span>
              </div>
              <div className="flex flex-col gap-2">
                <strong className="text-sm">Second Party Signature:</strong>
                <Image
                  src={(agreement as AgreementApi)?.second_party_signature || "/not-found-image.png"}
                  alt="First Party Signature"
                  width={20}
                  height={20}
                  className="w-[16em] h-[10em] bg-white object-cover rounded-lg"
