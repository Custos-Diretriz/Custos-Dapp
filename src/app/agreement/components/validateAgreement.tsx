...
const ValidateAgreementModal = ({
  ...
  onClose,
  agreement,
}: ValidateAgreementModalProps): React.JSX.Element => {
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      openNotification(
        "error",
        "",
        (err as Error).message || "Contract interaction failed"
      );
      setIsSuccess(false);
    } finally {
                ...
                <div className="py-2 px-4 border border-[#ffffff46] rounded-lg">
                  {agreement?.second_party_valid_id ? (
                    <Image
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = "/default-id-placeholder.png";
                      }}
                    />
                  ) : (
