...
const AgreementTemplate = (): React.JSX.Element => {
  const [selectedAgreement, setSelectedAgreement] = React.useState<any>(null);

  const handleItemSelect = (id: number): void => {
    setSelectedAgreement(agreementData.find((item) => item.id === id));
  };
    ...
    <div>
      {selectedAgreement ? (
        <AgreementEditCard
          ...
          onBack={() => setSelectedAgreement(null)} // Function to go back
        />
      ) 
      ...
      (
        <div className="w-full flex flex-col items-center py-8">
          ...
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-4">
            {agreementData.map(({ id, title, content }) => (
              <AgreementBox key={id} id={id} title={title} content={content} onSelect={handleItemSelect} />
            ))}
          </div>
        </div>
      )}
