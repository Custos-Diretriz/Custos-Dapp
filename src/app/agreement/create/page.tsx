...
const MappedSelect = ({ id, name, value, onChange, options, className }: MappedSelectProps): React.JSX.Element => {
  return (
    <select
      id={id}
      name={name}
...
export default function AgreementModal (): React.JSX.Element {
  ...
  const handleAgreementSelection = (agreement: any) => {
  ...
  const renderErrors = () => {
    return Object.values(errors).map((err: any, idx: number) => (
          ...
          <>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                name="idImage"
                accept="image/*"
                onChange={(e) => setIdImage(e.target.files?.[0]?.name || "")}
                className="mt-1 focus:outline-none w-full border-[#BEBDBD] focus-visible:top-10 focus:border-[#19B1D2] active:border-[#0094FF] px-2 py-3 rounded-md bg-transparent border shadow-sm text-white sm:text-sm"
              />
            </div>
