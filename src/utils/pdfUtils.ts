...
export const generatePDFWithWatermark = async (agreement: any, logoUrl = '/CustosLogo.png') => {
  const doc = new jsPDF();
...
export const printAgreement = async (agreement: any) => {
  try {
...
export const downloadAgreement = async (agreement: any) => {
  try {
    const doc = await generatePDFWithWatermark(agreement);
